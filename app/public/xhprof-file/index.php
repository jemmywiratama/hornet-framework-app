...
$PASSWORD = 'max123';  // Set the password, to access the file manager... (optional)
...
//$tmp_dir = '/data/www/hornet-framework-app/app/storage/xhprof'; 
define('APP_PATH', realpath( '../../' ) . '/');
$tmp_dir = APP_PATH.'storage/xhprof'; 

if(DIRECTORY_SEPARATOR==='\\') $tmp_dir = str_replace('/',DIRECTORY_SEPARATOR,$tmp_dir);

$tmp = get_absolute_path( $_REQUEST['file'] );
//
if($tmp === false)
	err(404,'File or Directory Not Found');
if( !empty( $_REQUEST['file'] ) &&  strpos($tmp,$tmp_dir)===false ){
	//var_dump('/'.$tmp,$tmp_dir);
	err(403,"Forbidden");
}
...
if($_GET['do'] == 'list') {
	if (is_dir($file)) {
	    ...
	    foreach($files as $entry) if($entry !== basename(__FILE__) && !in_array(strtolower(pathinfo($entry, PATHINFO_EXTENSION)), $hidden_extensions)) {
    		$i = $directory . '/' . $entry;
	    	$stat = stat($i);
			list($name_run_id,$name_source ) = explode('.', $entry);
	        $result[] = [
	        	'mtime' => $stat['mtime'],
	        	'size' => $stat['size'],
	        	'name' => basename($i),
	...
	$ret = move_uploaded_file($_FILES['file_data']['tmp_name'], $file.'/'.$_FILES['file_data']['name']);
	err(200,$ret);
	exit;
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		...
		var xhprof_link_url = '/xhprof/xhprof_html/?run='+encodeURIComponent(data.run_id)+'&source='+encodeURIComponent(data.source)+'&file='+encodeURIComponent(data.path);
		var $link = $('<a class="name" />')
			.attr('href', data.is_dir ? '#' + data.path : xhprof_link_url)
			.text(data.name);
			if(!data.is_dir){
		...
		var $xhprof_link = $('<a/>').attr('href', xhprof_link_url ).text('  xhprof');	
		$xhprof_link.attr('target','_blank');
		var $delete_link = $('<a href="javascript:;" />').attr('data-file',data.path).addClass('delete').text('delete');
		...
		var $html = $('<tr />')
			...
			.append( $('<td/>').append($dl_link).append($xhprof_link).append( data.is_deleteable ? $delete_link : '') )
		return $html;
	}
